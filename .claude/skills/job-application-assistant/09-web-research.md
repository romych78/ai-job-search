...
3. **The user's own browser** (Roman's Chrome, via the gstack `/browse` skill) on the target URL directly. `robots.txt` does **not** gate this step, by design, regardless of what step 2 found. robots.txt is the robots-exclusion protocol - it addresses automated crawlers, and it is what tells an honest bot (a `WebFetch`/`curl` call identifying itself as one) which paths it may not visit unattended. It says nothing about Roman opening a page in his own browser, because that is not what it is for. When Claude drives Roman's actual Chrome session to view a posting on his behalf, that is Roman browsing - assisted, but not automated crawling - the same act as if he'd opened the tab himself and read it aloud. Use this step whenever step 1/2 fails and the exact original URL matters (not just an equivalent canonical one); it makes step 4's URL-hunting unnecessary in most cases since it can just load the URL that's already known to be the posting.
4. **`WebSearch`** for the company or role by name, to find an alternative canonical URL, when the browser step above is unavailable (no connected browser) or impractical. The employer's own careers portal is almost always richer than the aggregator that surfaced the posting, and it carries the reference ID and grade that aggregators drop.
5. **Declare it genuinely unavailable** only after 1 to 4 have failed. In `/rank` that means marking the entry `expired`; in `/apply` it means telling the user the posting could not be retrieved and stopping rather than drafting from the title.
