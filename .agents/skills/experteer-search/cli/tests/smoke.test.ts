...
describe("Experteer CLI error contract (offline)", () => {
  ...
  test("a detail id with no numeric suffix fails before making a request", async () => {
...
// Live smoke tests — real requests against Experteer's public job search.
// Kept to a minimum (one search, one detail) per the repo's low-volume
// convention.
describe("Experteer CLI live smoke test", () => {
  test("search returns real, non-garbled results", async () => {
    const result = await runCLI(["search", "-q", "CTO", "--limit", "5", "--format", "json"]);
    const body = parseJSON<{ meta: { count: number; page: number; total: number | null }; results: any[] }>(
      result,
    );
    ...
    expect(first.url).toContain("experteer.de");
  }, 30000);
  ...
  test("detail resolves a real search result to its full description", async () => {
    ...
    if (job.description) {
      expect(job.description).not.toMatch(/<[a-z]+>/i); // no leaked HTML tags
    }
  }, 30000);
});
