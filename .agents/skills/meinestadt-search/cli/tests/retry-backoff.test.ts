...
describe("htmlFetch retry/backoff", () => {
  test("retries a 429 and succeeds on the next attempt", async () => {
    ...
    const html = await htmlFetch("https://jobs.meinestadt.de/deutschland/jkl/0-15214-15780");
  ...
  test("returns the documented empty string on 404 without retrying", async () => {
    ...
    const html = await htmlFetch("https://jobs.meinestadt.de/frankfurt-am-main/premium?id=0");
    expect(html).toBe("");
  ...
  test("gives up after the initial attempt plus six retries on persistent 5xx", async () => {
    ...
    await expect(htmlFetch("https://jobs.meinestadt.de/deutschland/jkl/0-15214-15780")).rejects.toThrow(/500/);
