...
describe("htmlFetch retry/backoff", () => {
  test("retries a 429 and succeeds on the next attempt", async () => {
    ...
    const html = await htmlFetch("https://www.stepstone.de/jobs/cto?q=CTO");
  ...
  test("returns the documented empty string on 404 without retrying", async () => {
    ...
    const html = await htmlFetch("https://www.stepstone.de/stellenangebote--job--0-inline.html");
    expect(html).toBe("");
  ...
  test("gives up after the initial attempt plus six retries on persistent 5xx", async () => {
    ...
    await expect(htmlFetch("https://www.stepstone.de/jobs/cto?q=CTO")).rejects.toThrow(/500/);
