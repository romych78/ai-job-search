...
describe("Xing CLI error contract (offline)", () => {
  ...
  test("an unresolvable detail id (no trailing numeric id) fails before making a request", async () => {
    const result = await runCLI(["detail", "not-a-valid-slug"]);
    expect(result.exitCode).toBe(1);
    expect(result.stdout).toBe("");
    const error = JSON.parse(result.stderr);
...
// Live smoke tests — real requests against Xing's public job search. Kept to
// a minimum (one search, one detail) per the repo's low-volume convention.
describe("Xing CLI live smoke test", () => {
  test("search returns real, non-garbled results", async () => {
    ...
    expect(first.url).toContain("xing.com/jobs/");
  }, 30000);
